...
class Doctor {
    ...
    $sql = "SELECT d.*, s.spec_name
            FROM {$this->table_doctor} d
            LEFT JOIN specialization s ON d.spec_id = s.spec_id
            WHERE d.doc_first_name LIKE :keyword
               OR d.doc_last_name LIKE :keyword
            ORDER BY d.doc_last_name ASC";
    $stmt = $this->conn->prepare($sql);
    ...
    public function searchDoctorsById($id) {
        $sql = "SELECT d.*, s.spec_name
                FROM {$this->table_doctor} d
                LEFT JOIN specialization s ON d.spec_id = s.spec_id
...
public function getBySpecialization($spec_id) {
    $sql = "SELECT * FROM {$this->table_doctor} WHERE spec_id = :spec_id ORDER BY doc_last_name ASC";
    $stmt = $this->conn->prepare($sql);
...
?>
        ...
        <!-- <?php 
          // Use doctor's image if available, otherwise use default placeholder
       //   $imagePath = !empty($row['doc_image'])
        //    ? "../uploads/doctors/" . htmlspecialchars($row['doc_image'])
        //    : "";
        //    require "../"
        ?> -->