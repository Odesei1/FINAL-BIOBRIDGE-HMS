...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['register'])) {
    ...
    if (!$email || !$user_name || !$password || !$confirm) {
        ...
        if (!$patient) {
            $modalMessage = "Email not found in records!";
            $modalType = 'error';
            $showRegister = true;
        } else {
            ...
            if ($check->fetch()) {
                ...
                $stmt->execute([$user_name, $password, $pat_id]);
                $modalMessage = "🎉 Account created successfully! You can now log in.";
                $modalType = 'success';
                $showRegister = true;
                $patientEmail = '';
                $userDisplay  = '';
...
<div id="container" class="relative flex w-full transition-section">
  ...
  <section id="patient-notice" class="absolute top-0 left-0 w-full min-h-screen flex items-center justify-center bg-gradient-to-r from-sky-100 to-white transition-section translate-left">
    <div class="bg-white shadow-xl rounded-2xl p-10 w-full max-w-lg text-center">
      ...
      <p class="text-gray-600 mb-6">Please tell us if you're an <span class="font-semibold">existing patient</span> or a <span class="font-semibold">new patient</span> so we can guide you properly.</p>
        ...
        <div class="shadow-2xl rounded-xl bg-white p-8 w-full max-w-md mx-auto">
          ...
          <div id="login-form">
            <form method="POST" action="Public/login_register.php" class="space-y-4" autocomplete="off">
              ...
              <p class="text-center text-sm mt-2">Don't have an online account? <a href="#" onclick="showForm('register')" class="text-sky-600 hover:underline font-medium">Link your patient record here</a></p>
            </form>
          </div>
          ...
          <div id="register-form" class="hidden">
            <form method="POST" class="space-y-4">
              ...
              <input type="email" name="username" placeholder="Enter your registered email" required
                     class="w-full p-2 bg-gray-100 border border-gray-300 rounded" value="<?= htmlspecialchars($patientEmail) ?>">

              <label class="block font-semibold text-gray-700">Username</label>
              <input type="text" name="user_display" placeholder="Choose a username" required
                     class="w-full p-2 bg-gray-100 border border-gray-300 rounded" value="<?= htmlspecialchars($userDisplay) ?>">
...
<!-- Modal -->
<div id="modal" class="fixed inset-0 hidden modal-bg flex items-center justify-center z-50">
  <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full text-center">
    <p id="modal-message" class="text-gray-700"></p>
    <button onclick="closeModal()" class="mt-4 bg-sky-600 hover:bg-sky-700 text-white px-4 py-2 rounded">OK</button>
  </div>
</div>
...
function showSection(id){
  ['landing','patient-notice','auth'].forEach(sec=>{
    ...
    if(sec===id){el.classList.remove('translate-left'); el.classList.add('translate-center');}
    else{el.classList.remove('translate-center'); el.classList.add('translate-left');}
  });
}
